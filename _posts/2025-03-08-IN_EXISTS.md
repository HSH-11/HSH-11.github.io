...
2. customer_id = 5와 1, 2, 3, 4 비교 결과는 TRUE지만, 5 != NULL 비교 결과는 UNKNOWN(즉, FALSE)
...
- EXISTS는 서브쿼리에서 해당하는 값을 **찾으면 즉시 종료**하므로, 서브쿼리의 결과 크기가 클수록 성능이 유리하다.
...
위 쿼리에서는 customers.customer_id가 NULL인 경우에도 결과가 존재하면 TRUE를 반환하여 해당 행이 출력 된다.
...
NOT EXISTS는 **서브쿼리 결과가 존재하지 않는 경우** TRUE를 반환하여 해당 행을 출력함.
...
orders 테이블에서 customer_id가 customers 테이블에 존재하지 않는 경우만 반환됨.

<br>

### NULL 값이 포함된 경우의 동작

NOT EXISTS는 NULL 값이 포함된 서브쿼리에서도 **정상적으로 동작**함.
...
orders.customer_id가 NULL인 경우에도 정상적으로 비교됨.

customers.customer_id에 NULL 값이 있어도 NOT EXISTS는 단순히 서브쿼리가 결과를 반환하는지만 확인하므로 문제 발생하지 않음.
...
이렇게 하면 customers.customer_id가 NULL인 경우 비교 대상에서 제외됨.
